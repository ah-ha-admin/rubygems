  ...
  context "with configured credentials" do
    it "adds username and password to URI" do
      Bundler.settings.temporary(uri => "u:p") do
        ...
        expect(subject).to receive(:git_retry).with("clone", "--bare", "--quiet", "--no-tags", "--depth", "1", "--single-branch", "--", "https://u:p@github.com/rubygems/rubygems.git", path.to_s)
        subject.checkout
      end
    end
    ...
    it "does not add username and password to mismatched URI" do
      Bundler.settings.temporary("https://u:p@github.com/rubygems/rubygems-mismatch.git" => "u:p") do
        allow(subject).to receive(:git).with("--version").and_return("git version 2.14.0")
        expect(subject).to receive(:git_retry).with("clone", "--bare", "--quiet", "--no-tags", "--depth", "1", "--single-branch", "--", uri, path.to_s)
        subject.checkout
      end
    end
    ...
    it "keeps original userinfo" do
      Bundler.settings.temporary("github.com" => "u:p") do
        ...
        allow(subject).to receive(:git).with("--version").and_return("git version 2.14.0")
        expect(subject).to receive(:git_retry).with("clone", "--bare", "--quiet", "--no-tags", "--depth", "1", "--single-branch", "--", original, path.to_s)
        subject.checkout
      end
    end
  end
